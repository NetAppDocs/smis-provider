...
[.lead]
NetApp SMI-S Provider might not respond to a query when no storage system has been added to the CIMOM repository. Learn how to verify whether a storage system is registered and add one if needed.
