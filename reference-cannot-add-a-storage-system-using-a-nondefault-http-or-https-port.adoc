...
[.lead]
You cannot add a NetApp storage system to NetApp SMI-S Provider when that system uses HTTP or HTTPS on a nondefault port. Add the storage system by specifying the nondefault port explicitly.
