...
[.lead]
NetApp SMI-S Provider can fail to connect to `localhost:5989` when HTTPS connections are disabled, the HTTPS port is misconfigured, or the provider has stopped responding. Use this information to verify your HTTPS connection settings and restart the provider to resolve the issue.
